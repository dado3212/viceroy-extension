import {
  ...
  syncHeaders,
  getHeader,
  updateHeadersFromCache,
  Header,
} from '@/lib/headers';
import { fetchUberEats } from '@/lib/apis/eats';
import { fetchUberRides } from '@/lib/apis/rides';
import { fetchBayWheels } from '@/lib/apis/baywheels';
import {
  getPendingTransactions,
  applyMonarchDecision,
  fetchMonarchTags,
  monarchLogs,
} from '@/lib/apis/monarch';
import type {
  UberRide,
  UberEatsOrder,
  BayWheelsRide,
...
export default defineBackground(() => {
  // Initialize the header listener for intercepting auth headers
  setupHeaderListener();
  ...
  function getShortName(
    ...
    if (shortName !== undefined) {
      return [
        shortName,
        shortName === 'my house' || shortName === 'my old apartment'
          ? true
          : false,
      ];
    }
    if (
      waypoint.includes('San Francisco International Airport') ||
      waypoint.includes('San Francisco Airport')
    ) {
  ...
  function generateDescription(
    ...
    if (startName) {
      ...
      if (
        new Date().getTime() - new Date(uber.details!.startTime).getTime() >
        86400000 * 30 * 4
      ) {
    ...
    const [endName, isHome] = getShortName(
      locations,
    ...
    if (waypoints.length > 2) {
      description +=
        ...
        waypoints
          ...
          .map((x) => getShortName(locations, x)[0] ?? x)
          .join(' and ');
    }
    return description;
  }
  ...
  async function matchDataToTxns(
    ...
    const { locations = {} }: { locations: { [key: string]: string } } =
      await chrome.storage.sync.get('locations');
    ...
    uberRides.forEach((u) => {
      ...
      const pushUSD = (
        amtStr: string,
        opts: Partial<AnnotatedUberRide['_norm']> = {}
      ) => {
        const ann: AnnotatedUberRide = {
          ...u,
          _norm: {
            amount: amtStr,
            date,
            description: opts.description ?? baseDesc,
            isTip: opts.isTip,
          },
        };
        usdPool.push({ ...ann, _amtNum: Number(amtStr) });
      };
      const pushOtherCurrency = (
        amtStr: string,
        opts: Partial<AnnotatedUberRide['_norm']> = {}
      ) => {
        diffCurrency.push({
      ...
      if (origAmount.startsWith('$')) pushUSD(amtStr);
      else pushOtherCurrency(amtStr);
      ...
      if (origAmount !== u.details!.fare) {
        ...
        if (origAmount.startsWith('$')) pushUSD(combined);
        else pushOtherCurrency(combined);

        // Just tip (difference of the two)
        const tipValue =
          parseInt(u.details!.fare.replace(/^[^\d]/, '')) -
          parseInt(origAmount.replace(/^[^\d]/, ''));
    ...
    // For Uber Eats
    const uberEatsPool: Array<
      UberEatsOrder & { _amtNum: number; isTip: boolean; note: string }
    > = [];
    uberEats.forEach((u) => {
    ...
    const pickBestEats = (
      cands: Array<AnnotatedUberEatsOrder>,
      t: MonarchTransaction
    ): AnnotatedUberEatsOrder | null => {
    ...
    const pickBestBaywheelsRides = (
      t: MonarchTransaction
    ): Array<AnnotatedBayWheelsRide> => {
      const [, rideNum, date] =
        t.dataProviderDescription.match(/\*(\d+)\s+RIDES?\s+(\d{1,2}-\d{1,2})/) ||
        [];
      ...
      if (!rideNum || parseInt(rideNum) === 1) {
        ...
        if (Math.abs(total - t.amount) < 1e-9) {
          return sameDay.sort(
            (a: AnnotatedBayWheelsRide, b: AnnotatedBayWheelsRide) =>
    ...
    txns.forEach((t) => {
      ...
      const desc = (t.dataProviderDescription || '').toUpperCase();
      ...
      if (desc.includes('UBER') && desc.includes('EATS')) {
        // Uber Eats - check EATS first since it's more specific
        // Exact match
        bestEats = pickBestEats(
          uberEatsPool.filter((u) => u._amtNum === t.amount),
          t
        );

        // Some window of 10%
        if (!bestEats) {
          const tol = Math.max(0.5, Math.abs(t.amount) * 0.1); // up to $0.50 is fine
          bestEats = pickBestEats(
            uberEatsPool.filter((u) => Math.abs(u._amtNum - t.amount) <= tol),
            t
          );
          if (bestEats) {
            warn = true;
        ...
        // Any UBER that's not EATS is treated as a RIDE
        // This catches: "UBER TRIP", "UBER", "PAYPAL *UBER", "Uber *lime", etc.
        bestRide = pickBestRide(
          usdPool.filter((u) => u._amtNum === target),
          t
        );

        if (!bestRide) {
          const tol = Math.max(2.5, Math.abs(target) * 0.1); // up to $2.50 is fine
          bestRide = pickBestRide(
            usdPool.filter((u) => Math.abs(u._amtNum - target) <= tol),
            t
          );
      ...
      } else if (desc.includes('LYFT')) {
        // LYFT - match with BayWheels/Lyft scooter data
        bestBayWheelsRides = pickBestBaywheelsRides(t);

        if (bestBayWheelsRides.length > 0) {
          const rideText = bestBayWheelsRides.map(
            (r: AnnotatedBayWheelsRide) => {
        ...
        // Skip transactions that don't match known patterns
        console.log('[Background] Skipping unknown transaction:', t.dataProviderDescription);
        return;
      }
  ...
  chrome.runtime.onConnect.addListener(async (port) => {
    // Port name format: "fetch:1" or "fetch:0" where 1=needsReview true, 0=false
    if (!port.name.startsWith('fetch:')) return;

    // Parse needsReview from port name
    const needsReview = port.name === 'fetch:1';
    console.log('[Background] Port connected:', port.name, 'needsReview:', needsReview);

    try {
      port.postMessage({ progress: 'Checking headers…' });
      await syncHeaders();

      // Only Monarch is required - other services are optional
      const monarchHeader = getHeader(Header.Monarch);
      if (!monarchHeader) {
        const headerStatus: Partial<Record<Header, Record<string, string>>> = {};
        for (const headerName of Object.values(Header)) {
          headerStatus[headerName] = getHeader(headerName);
        }
      ...
      // Build debug info for service connections
      const hasUberRides = getHeader(Header.UberRides) != null;
      const hasUberEats = getHeader(Header.UberEats) != null;
      // const hasBayWheels = getHeader(Header.BayWheels) != null; // Disabled for now

      const debug: {
        monarchTotal: number;
        uberRidesCount: number;
        uberEatsCount: number;
        services: { monarch: boolean; uberRides: boolean; uberEats: boolean };
        uberRidesFetched?: number;
        uberEatsFetched?: number;
      } = {
        monarchTotal: 0,
        uberRidesCount: 0,
        uberEatsCount: 0,
        services: {
          monarch: true,
          uberRides: hasUberRides,
          uberEats: hasUberEats,
        },
      };

      // Fetch from all connected services FIRST (independently of Monarch)
      port.postMessage({ progress: 'Fetching from connected services…' });
      const lookbackDays = 90;
      const now = Date.now();
      const oldestDate = now - 60 * 60 * 24 * 1000 * lookbackDays;

      const [uberRides, uberEats] = await Promise.all([
        hasUberRides ? fetchUberRides(200, now, oldestDate) : [],
        hasUberEats ? fetchUberEats(oldestDate) : [],
        // BayWheels disabled for now
      ]);
      const bayWheels: BayWheelsRide[] = []; // Empty array for now

      // Add fetched counts to debug immediately
      debug.uberRidesFetched = uberRides.length;
      debug.uberEatsFetched = uberEats.length;

      // Send debug info early so user can see what was fetched
      port.postMessage({ debug });

      // Now fetch Monarch transactions
      console.log('[Background] About to fetch Monarch transactions with needsReview =', needsReview);
      port.postMessage({ progress: needsReview ? 'Fetching pending transactions from Monarch…' : 'Fetching all transactions from Monarch…' });
      const lookback = 200;
      const pending = await getPendingTransactions({ limit: lookback, needsReview });
      console.log('[Background] Monarch returned', pending.length, 'transactions');

      // Count transactions by type for debugging
      // EATS is checked first since it's more specific
      const uberEatsTransactions = pending.filter((x) => {
        const desc = (x.dataProviderDescription || '').toUpperCase();
        return desc.includes('UBER') && desc.includes('EATS');
      });
      // RIDES = any UBER that's not EATS (catches TRIP, PAYPAL *UBER, Uber *lime, etc.)
      const uberRideTransactions = pending.filter((x) => {
        const desc = (x.dataProviderDescription || '').toUpperCase();
        return desc.includes('UBER') && !desc.includes('EATS');
      });
      // LYFT transactions
      const lyftTransactions = pending.filter((x) => {
        const desc = (x.dataProviderDescription || '').toUpperCase();
        return desc.includes('LYFT');
      });

      // Update debug with Monarch counts
      debug.monarchTotal = pending.length;
      debug.uberRidesCount = uberRideTransactions.length;
      debug.uberEatsCount = uberEatsTransactions.length;

      port.postMessage({ progress: 'Matching transactions…' });
      port.postMessage({
        data: await matchDataToTxns(uberRides, uberEats, bayWheels, pending),
        debug,
        logs: monarchLogs.slice(), // Send a copy of the logs
      });
  ...
  // Return actual header objects for export/debugging
  chrome.runtime.onMessage.addListener((msg, _s, sendResponse) => {
    (async () => {
      if (msg.type !== 'fetchHeadersForExport') return;
      await updateHeadersFromCache();

      const headerData: Partial<Record<Header, Record<string, string>>> = {};
      for (const headerName of Object.values(Header)) {
        const h = getHeader(headerName);
        if (h) {
          headerData[headerName] = h;
        }
      }

      sendResponse({ data: headerData });
    })().catch((err) =>
      sendResponse({ error: String((err as Error)?.message || err) })
    );
    return true;
  ...
  chrome.runtime.onMessage.addListener((msg, _s, sendResponse) => {
    (async () => {
      ...
      const { tags = {} }: { tags: { [key: string]: MonarchTag & { checked: boolean } } } =
        await chrome.storage.sync.get('tags');
  ...
  chrome.runtime.onMessage.addListener((msg, _s, sendResponse) => {
    (async () => {
      ...
      for (const monarchTag of monarchTags) {
        newTags[monarchTag.id] = {
          ...monarchTag,
          checked: tags[monarchTag.id]?.checked ?? false,
        };
      }
